...
import { SignUpURL } from "@/constants/apiConstants";
...
export default function SignUp() {
  ...
  const handleSignUp = async (data: FormValues) => {
    const resp = await POSTRequest(SignUpURL, data);
  ...
  const signUpMutation = useMutation({
    ...
    onSuccess: (data) => {
      if (data?.success) {
        toast.success(data.message ?? "Please verify your OTP to continue");
        setverifyOtp(true);
        return;
      }
    ...
    onError: (error?: { response?: { data?: { message?: string } } }) => {
      toast.error(
        error?.response?.data?.message ?? "SignUp failed unexpectedly"
      );
    },
  });
      ...
      {verifyOtp ? (
        ...
        <form className="my-8" onSubmit={handleSubmit(onSubmit)}>
          ...
          <button
            ...
            type="submit"
          >
            {signUpMutation.isPending ? "Signing up" : "Sign Up"} &rarr;
            <BottomGradient />
          </button>
